...
class GUIControls:
    ...
    def setup_car_selector(self):
        """Set up car selection and priority controls using simple buttons"""
        # Get number of cars
        num_cars = len(self.sim.cars)
        
        # Add car selection label
        self.ax_car_label = plt.axes([0.45, 0.22, 0.08, 0.03])
        self.ax_car_label.text(0.5, 0.5, "Car:", ha='right', va='center')
        self.ax_car_label.axis('off')
        
        # Add car selection buttons
        # Place up to 3 buttons in a row
        button_width = 0.03
        ...
        for i in range(num_cars):
            ...
            x_pos = 0.53 + col * (button_width + button_spacing)
            y_pos = 0.22 - row * 0.04
            
            ax_btn = plt.axes([x_pos, y_pos, button_width, 0.03])
            btn = Button(ax_btn, f"{i}", color=self.sim.cars[i].color)
            btn.on_clicked(lambda event, idx=i: self.select_car(idx))
            self.car_buttons.append(btn)
        
        # Car priority controls - placed below the car selection
        self.ax_priority_label = plt.axes([0.45, 0.16, 0.08, 0.03])
        self.ax_priority_label.text(0.5, 0.5, "Priority:", ha='right', va='center')
        self.ax_priority_label.axis('off')
        
        # Priority control buttons
        self.ax_priority_up = plt.axes([0.53, 0.16, 0.03, 0.03])
        ...
        self.ax_priority_down = plt.axes([0.57, 0.16, 0.03, 0.03])
        ...
        # Add text to show selected car
        self.selected_car_text = self.ax.text(0.5, 0.27, f"Selected: Car {self.selected_car_idx}", 
    ...
    def select_car(self, idx):
        ...
        # Update the selected car text
        self.selected_car_text.set_text(f"Selected: Car {self.selected_car_idx}")
    ...
    def setup_sliders(self):
        ...
        # Car speed slider
        self.ax_car_speed = plt.axes([0.45, 0.1, 0.15, 0.03])
