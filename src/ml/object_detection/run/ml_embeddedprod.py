...
class VehicleTrackerApp:
    def __init__(self, root):
        ...
        self.yolov9_model = YOLO("./config/yolov9s.pt").to(self.device) #switched over to yolov9s model for less memory usage
...
#########CODE BELOW IS TO CHECK MEMORY CONSUMPTION
# import torch
# import cv2
# import numpy as np
# import psutil
...
# def memory_usage():
#     """Returns current RAM and GPU memory usage in MB."""
...
#     return f"RAM: {ram_usage:.2f} MB, GPU: {gpu_usage:.2f} MB"
...
#         print(f"Memory usage before loading YOLO model: {memory_usage()}")
#         self.yolov9_model = YOLO("./config/yolov9s.pt").to(self.device)
#         print(f"Memory usage after loading YOLO model: {memory_usage()}")

#         print("Loading classification model...")
#         self.classification_model = models.resnet18(weights='IMAGENET1K_V1')
#         self.classification_model.fc = nn.Linear(self.classification_model.fc.in_features, num_classes)
#         self.classification_model.load_state_dict(torch.load("./CNNModels/best.pt", map_location=self.device))
#         self.classification_model.to(self.device)
#         self.classification_model.eval()
#         print(f"Memory usage after loading classification model: {memory_usage()}")
...
#             print(f"Memory before inference: {memory_usage()}")
#             results = self.yolov9_model.predict(img_rgb, classes=[2], verbose=False, imgsz=480)
#             print(f"Memory after inference: {memory_usage()}")
...
#     def on_closing(self):
#         print(f"Memory before shutdown: {memory_usage()}")
#         if self.cap.isOpened():
#             self.cap.release()
#         cv2.destroyAllWindows()
#         self.root.destroy()
#         print(f"Memory after shutdown: {memory_usage()}")
