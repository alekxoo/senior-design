...
class VehicleTrackerApp:
    ...
    def __init__(self, root):
        ...
        self.grabber_thread = threading.Thread(target=self.frame_grabber, daemon=True)
        self.grabber_thread.start()

        # Schedule the first update of the webcam frame
        self.create_ui()
        self.up_thread = threading.Thread(target=self.update_frame, daemon=True)
    ...
    def record_and_save(self):
        ...
        if not self.is_recording:
            ...
            self.video_writer = cv2.VideoWriter(self.output_path, fourcc, 59.99, (1920, 1080))
    ...
    def update_frame(self):
        while True:
            old_time = time.time()
            img = self.newest_frame
            if img is None:
                continue #no newest frame yet...
            ...
            if self.tracking_enabled:
                ...
                PID(0.0, 0.0, 0.0, False) #make sure timer is still running

            new_time = time.time()
            print("update_frame: ", new_time - old_time)
